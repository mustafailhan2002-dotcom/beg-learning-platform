...
export default async function TeacherRevisionsPage() {
  const items = (await getTeacherReviewItems()).filter((item) => item.status === "revision_required");
      ...
      <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(250,204,21,0.16),transparent_24%),linear-gradient(135deg,#07111f_0%,#0f1f3d_55%,#07111f_100%)] px-4 py-6 text-slate-100 sm:px-6 lg:px-8 lg:py-10">
        <div className="mx-auto max-w-7xl rounded-[2rem] border border-slate-800/80 bg-slate-950/80 p-6 shadow-2xl shadow-slate-950/50 backdrop-blur sm:p-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.35em] text-amber-400">Revision Requests</p>
              <h1 className="mt-3 text-3xl font-semibold text-white sm:text-4xl">Content returned for revision</h1>
            </div>
          ...
          <div className="mt-8 space-y-4">
            ...
            )) : <div className="rounded-3xl border border-dashed border-slate-700 bg-slate-950/40 p-8 text-center text-slate-400">No revisions pending.</div>}
          </div>
        </div>
      </main>
    </ProtectedRoute>
