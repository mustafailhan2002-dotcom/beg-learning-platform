...
import { lessons } from "@/lib/mathematics/data";
  ...
  return (
    <ProtectedRoute>
      <MathematicsShell
        ...
        description="Create and manage IGCSE Mathematics lessons with objectives, examples, practice, homework and teacher notes."
        activePath="/mathematics/lessons"
      >
        {lessons.length > 0 ? (
            ...
            {lessons.map((lesson) => (
              <article key={lesson.id} className="rounded-3xl border border-slate-800 bg-slate-900/70 p-6">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <p className="text-sm font-semibold uppercase tracking-[0.3em] text-amber-400">{lesson.id}</p>
                    <h2 className="mt-2 text-xl font-semibold text-white">{lesson.title}</h2>
                    <p className="mt-2 text-sm text-slate-400">Unit: {lesson.unit}</p>
                  </div>
                ...
                <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-950/60 p-4">
                  ...
                  <p className="mt-2 text-sm leading-7 text-slate-400">{lesson.teacherNotes}</p>
                </div>
              </article>
            ))}
          </div>
